...
void UDialog::initialize()
{
   ...
   U_INTERNAL_ASSERT_EQUALS(path_dialog, U_NULLPTR)
...
bool UDialog::passwordbox(const char* ptext, UString& init, const char* title)
{
   U_TRACE(0, "UDialog::passwordbox(%S,%V,%S)", ptext, init.rep, title)

   if (run(title, "--passwordbox \"%s\" %d %d", ptext, height, width))
      {
      init = output.copy();
...
int UDialog::radiolist(const char* ptext, const char* tags[], const char* items[], int select, int list_height, const char* title)
{
   U_TRACE(0, "UDialog::radiolist(%S,%p,%p,%d,%d,%S)", ptext, tags, items, select, list_height, title)
   ...
   if (run(title, "--single-quoted --radiolist \"%s\" %d %d %d", ptext, height, width, list_height) &&
       output)
      {
...
void UDialog::checklist(const char* ptext, const char* tags[], const char* items[], bool selected[], int list_height, const char* title)
{
   U_TRACE(0, "UDialog::checklist(%S,%p,%p,%p,%d,%S)", ptext, tags, items, selected, list_height, title)
   ...
   if (run(title, "--single-quoted --checklist \"%s\" %d %d %d", ptext, height, width, list_height) &&
       output)
      {
      UVector<UString> vec;
...
int UDialog::menu(const char* ptext, const char* tags[], const char* items[], int list_height, const char* title)
{
   U_TRACE(0, "UDialog::menu(%S,%p,%p,%d,%S)", ptext, tags, items, list_height, title)
   ...
   if (run(title, "--single-quoted --menu \"%s\" %d %d %d", ptext, height, width, list_height) &&
       output)
      {
      for (int j = 0; tags[j]; ++j)
         {
...
bool UDialog::menu(const char* ptext, UVector<UString>& list, UString& choice, int list_height, const char* title)
{
   U_TRACE(0, "UDialog::menu(%S,%p,%p,%d,%S)", ptext, &list, &choice, list_height, title)
...
bool UDialog::inputbox(const char* ptext, UString& init, const char* title)
{
   U_TRACE(0, "UDialog::inputbox(%S,%V,%S)", ptext, init.rep, title)

   if (run(title, "--inputbox \"%s\" %d %d \"%v\"", ptext, height, width, init.rep))
      {
      init = output.copy();
...
bool UDialog::inputsbox(int n, const char* ptext, const char* labels[], UVector<UString>& init, const char* title)
{
   U_TRACE(0, "UDialog::inputsbox(%d,%S,%p,%p,%S)", n, ptext, labels, &init, title)
   ...
   if (xdialog &&
      ...
      if (run(title, "--%dinputsbox \"%s\" %d %d", n, ptext, height, width))
         {
...
bool UDialog::combobox(const char* ptext, UVector<UString>& list, UString& choice, const char* title)
{
   U_TRACE(0, "UDialog::combobox(%S,%p,%p,%S)", ptext, &list, &choice, title)
   ...
   if (run(title, "--combobox \"%s\" %d %d", ptext, height, width))
      {
      choice = output.copy();
...
int UDialog::tree(const char* ptext, const char* items[], uint32_t items_depth[], const char* item_tags[],
                  const char* items_status[], const char* items_help[], int list_height, const char* title)
{
   U_TRACE(0, "UDialog::tree(%S,%p,%p,%p,%p,%p,%d,%S)", ptext, items, items_depth, item_tags, items_status, items_help, list_height, title)
   ...
   if (run(title, "--treeview \"%s\" %d %d %d", ptext, height, width, list_height) &&
       output)
      {
      if (item_tags)
         {
