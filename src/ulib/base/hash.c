...
static uint32_t murmurhash3_x86_32(unsigned char* restrict bp, uint32_t len, bool ignore_case)
{
   ...
   U_INTERNAL_TRACE("murmurhash3_x86_32(%.*s,%u,%d)", U_min(len,128), bp, len, ignore_case)
...
__pure uint32_t u_hash(unsigned char* restrict bp, uint32_t len, bool ignore_case)
{
...
#     else
      if (len > 16)
         ...
         const uint32_t c1 = 0x239b961b,
                        ...
                        c3 = 0x38b34ae5,
                        c4 = 0xa1e38b93;
         uint32_t h1, h2, h3, h4, k1, k2, k3, k4;

         const uint8_t* tail    = (const uint8_t*)(bp + nblocks*16);
         const uint32_t* blocks = (const uint32_t*)tail;

         h1 = h2 = h3 = h4 = u_seed_hash; /* seed */
      ...
      return murmurhash3_x86_32(bp, len, false);
#     endif
#  endif
      }

   return murmurhash3_x86_32(bp, len, true);
}
