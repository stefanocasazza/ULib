...
void UCommand::setCommand()
{
   ...
   if (ncmd == U_NOT_FOUND)
      ...
      argv_exec = (char**) UMemoryPool::pmalloc(n + U_ADD_ARGS, sizeof(char*)); // U_ADD_ARGS => space for addArgument()...
...
uint32_t UCommand::setEnvironment(const UString& env, char**& _envp)
{
   ...
   _envp = (char**) UMemoryPool::pmalloc(n, sizeof(char*)); // NB: consider also null terminator...
