...
#	ifdef DEBUG
		if (U_SYSCALL(PQsendQueryPrepared, "%p,%S,%u,%p,%p,%p,%u", conn, pstmt->stmtName, 1, pstmt->paramValues, pstmt->paramLengths, pstmt->paramFormats, 1)  != 1)
			{
			U_ERROR("PQsendQueryPrepared() fail: %S", PQerrorMessage(conn));
			}
#	else
